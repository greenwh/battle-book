...
Absorbed the August 17, 2026 game update and reconciled several pages against a live Fire Crystal Age account.

- **Crystal Laboratory — new page.** Full refinement cost ladder, the 2.1 Fire Crystals expected per refine, and the efficiency cliff that makes the sixth refine cost ten times the first for the same return. Covers Super Refinement and why the 100-per-week cap is a ceiling rather than a target. The widely-cited slot-gating table (5 at FC5, 6 with War Academy, 7 at FC8, 8 at FC10) does not match an observed FC3 account; the conflict is documented rather than resolved.
- **Wish Station — new page.** The Fire Crystal Age city-development layer: Wishes, Wish Sigils, the Wish Shop, and the nine entertainment buildings, plus the Chief Order reshuffle that arrives with it. Announced to unlock at FC1 but observed at FC3, with an FC2 account on the same state lacking it — both figures published. Sigil rates and building effects are unpublished and deliberately left blank.
- **Combat Pets — corrected.** The page previously claimed 7 of 8 combat pets were unlocked on Gen 2 servers with Snow Leopard outstanding. Both halves were wrong. Rewritten around the **pet unlock chain**, which gates pets by leveling their predecessor and can run ahead of the generation calendar. Added the **Strengthening Serum ceiling** (do not push past level 50 until reserves allow) and corrected the Wild Mark rule, which sorts by **refinement stage, not by pet** — Commons for gray-to-blue on everything, Advanced reserved exclusively for purple-to-gold.
- **Alliance Championship pet activation changed.** Pet skills now activate **automatically at squad deployment**, calculated from pet skill level. The old "activate before you register" rule no longer applies to pets. Gem and facility buffs are still snapshotted at registration. Updated on Pet Buffs, Combat Pets, and the Pet Hub.
- **Server Timeline corrected.** The Crystal Laboratory was listed at Day 150 with FC5; it actually opens with the Fire Crystal Age at Furnace 30. Expert unlock rows added to the master timeline. Gen 4 heroes (Day 195) and Experts Valeria, Baldur, and Fabian (Day 200) confirmed against a live Gen 2 state. Added a warning that pet generation days are "available by" rather than "not before" — Iron Rhino unlocked at Day 200, eighty days ahead of its listed row.
- **Hero Gear — Mastery Forging section added.** Mastery Lv. 4 and above now upgrades in **sub-stages** rather than full levels, with total cost unchanged and a progress bar added. Partial progress toward a Mastery threshold is now visible and bankable.
- **Chief Gear** — Legendary T6 (3-Star) noted as available in states that have unlocked Generation 6 heroes.
- **Chief Charms** — Levels 17 and 18 added for Generation 8 states, nine sub-stages each. Separately, the **Frostdragon Tyrant Charm Secrets exchange is now gated at Fire Crystal 8**. That is a different shop from the Level 11 Chief Charm Material Exchange, whose unlock condition **remains unresolved** — the page now says so explicitly to prevent the two being conflated.
- **Alliance War** — Auto-Join Rally can no longer be enabled above 90% Infirmary capacity, and switches itself off if you cross the line. Added to Rally Joining and Troop Replenishment.
- **Bear Trap** — scheduled Hunting Trap activations now survive the scheduling member leaving the alliance.
- **Winter Siege expanded** after a completed event. Stronghold Star values, Heart totals, and garrison capacity are now published as observed figures. Added the four personal leaderboards, Stronghold Conquest Rewards, the Stars-per-attack targeting table, attacker tiering, the turnout argument, and the finding that the Empowered centre opens once any one outer is cleared.
...
- Branding carries forward from the State 4104 SvS guide (dark theme, ice-blue + amber accents, Georgia serif headings)
- Content templates established for events, heroes, combat references, hub pages, and reference data
- Initial content:
  - **SvS Overview** — full event guide ported from the State 4104 SvS guide
...
*See an error or want a guide written? Open an issue on [GitHub](https://github.com/greenwh/battle-book/issues) or message Bill in-game.*
